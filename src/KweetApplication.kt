...
class StudentAPI {
    ...
    @Location("books/{clazz}")
    class Books(val clazz: Int = 0)
...
//        postNew(getDao, hashFunction)
        ...
        if (!teacherDao.hasTeacher("meghdut.windows@gmail.com")) {
            teacherDao.addTeacher(
                Teacher(
                    "meghdut.windows@gmail.com", hash("meghdut.windows@gmail.com", "meghdut"),
...
suspend fun PipelineContext<Unit, ApplicationCall>.authFail() {
    call.respond(HttpStatusCode.Unauthorized, StringResponse(401, "Authoriazation failed"))
    // nice change
}

fun PipelineContext<Unit, ApplicationCall>.withTeacher(): Teacher? {
    return call.authentication.principal<JWTPrincipal>()
        ?.run {
            val idTeacher = payload.getClaim("id").asString()
            if (teacherDao.hasTeacher(idTeacher))
                teacherDao.getTeacher(idTeacher)
            else null
        }
}
